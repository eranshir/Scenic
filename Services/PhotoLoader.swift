...
class PhotoLoader: ObservableObject {
    ...
    func loadImage(from cdMedia: CDMedia, targetSize: CGSize = CGSize(width: 400, height: 400)) async -> UIImage? {
        ...
        if let url = cdMedia.url, !url.isEmpty {
            if let mediaId = cdMedia.id, let filename = await photoCache.downloadAndCache(from: url, mediaId: mediaId, isVideo: cdMedia.type == "video") {
    ...
    func loadThumbnail(from cdMedia: CDMedia) async -> UIImage? {
        ...
        if let thumbnailUrl = cdMedia.thumbnailUrl, !thumbnailUrl.isEmpty {
            if let mediaId = cdMedia.id, let filename = await photoCache.downloadAndCacheThumbnail(from: thumbnailUrl, mediaId: mediaId) {
                // Update Core Data
    ...
    func cacheFromPhotoLibrary(asset: PHAsset, cdMedia: CDMedia) async -> Bool {
        ...
        let filename: String?
        if isVideo {
            filename = await photoCache.cacheVideo(from: asset, mediaId: cdMedia.id ?? UUID())
        } else {
            filename = await photoCache.cachePhoto(from: asset, mediaId: cdMedia.id ?? UUID())
        }
        
        guard let filename = filename else {
            print("Failed to cache media for ID: \(cdMedia.id?.uuidString ?? "unknown")")
            return false
        }
        
        // Cache thumbnail
        let thumbnailFilename = await photoCache.cacheThumbnail(from: asset, mediaId: cdMedia.id ?? UUID())
    ...
    func cacheFromPhotosPicker(item: PhotosPickerItem, cdMedia: CDMedia) async -> Bool {
        ...
        guard let mediaId = cdMedia.id else {
            print("❌ CDMedia has no ID")
            return false
        }
